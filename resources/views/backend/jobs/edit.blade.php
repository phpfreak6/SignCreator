    ...
    <div class="content-center">
        <div class="container">
           @if ($errors->any())
			...
			@endif
			 {{ html()->modelForm($job, 'PATCH', route('backend.jobs.update',$job->id))->class('form-horizontal')->open() }}
					...
					<div class="form-group row">
                        {{ html()->label(__('jobs.installation_pic_check'))->class('col-md-4 form-control-label')->for('install_pic_check') }}
                         <div class="col-md-8 imm_dash_img">
						 <?php
						if (! empty($job->file)) {
					...
					<div class="form-group row">
						...
						<?php
						$ff = \App\Models\Job::selectedJobpreferreddate($job->id);
						?>
						 <div class="col-md-8">
						    <div class="install_error" style="color:red;display:none"></div>
							<input type="text" name="preferred_install_date" value="<?php echo $ff['date']; ?>" class="form-control preferred_install_date" id="preferred_install_date" placeholder="dd/mm/yyyy"> 
						</div>
					</div><!--form-group-->
