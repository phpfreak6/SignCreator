...
class LoginController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        if (Auth::attempt([
            ...
            if (! empty($current_role[0])) {
                if ($current_role[0] == "super admin") {
                    return redirect('admin/dashboard');
                } elseif ($current_role[0] == "print") {
                    return redirect('admin/dashboard');
                } else {
    ...
    protected function sendFailedLoginResponse(Request $request)
    {
        ...
        if ($user && \Hash::check($request->password, $user->password) && $user->confirmed != 1) {
            $errors = [
                $this->username() => __('exceptions.frontend.auth.confirmation.resend', [
                    'user_id' => $user->id
                ])
            ];
        }
    ...
    private function findOrCreateUser($socialUser, $provider)
    {
        ...
        if ($authUser = UserProvider::where('provider_id', $socialUser->getId())->first()) {
            ...
            $media = $authUser->addMediaFromUrl($socialUser->getAvatar())
